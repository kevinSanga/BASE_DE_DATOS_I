...
--los pacientes que tengan la m en sus nombres.
select count(pac.nombre)
from Paciente as pac
 inner join Factura as fac on pac.nombre = fac.nombre
where pac.nombre = 'm%'; 

--el numero de pacientes que sean menores a cierta edad
select count (pac.edad)
from paciente as pac
where pac.edad < 20;

--determinar la cantidad de pacientes que estan en cierta area.

SELECT COUNT(are.ID_AREA) 
FROM Area as are
	INNER JOIN Consulta as con on are.ID_AREA = con.ID_AREA
WHERE are.ID_AREA < '07';
